...
def fit_text_to_width_height(c, text, x, y, max_width, max_height, font_name,
    ...
    while (1):
        while (line_number * font_size > max_height) and font_size > 0:
            font_size -= 0.1
        if c.stringWidth(text, font_name,
                         font_size) > (line_number + 1) * max_width:
            line_number += 1
        else:
            break

    if (line_number + 1) * max_width > c.stringWidth(
            text, font_name, font_size) > line_number * max_width:
        font_size = min(
            font_size / (c.stringWidth(text, font_name, font_size) /
                         (line_number * max_width)), font_size)
        line_number += 1
    while (line_number * font_size > max_height) and font_size > 0:
        font_size -= 0.1
...
def fit_vertical_text_to_width_height(c, text, x, y, max_width, max_height,
    ...
    while (1):
        while (line_number * font_size > max_width) and font_size > 0:
            font_size -= 0.1
        if c.stringWidth(text, font_name,
                         font_size) > (line_number + 1) * max_height:
            line_number += 1
        else:
            break

    if (line_number + 1) * max_height > c.stringWidth(
            text, font_name, font_size) > line_number * max_height:
        font_size = min(
            font_size / (c.stringWidth(text, font_name, font_size) /
                         (line_number * max_height)), font_size)
        line_number += 1
    while (line_number * font_size > max_height) and font_size > 0:
        font_size -= 0.1
